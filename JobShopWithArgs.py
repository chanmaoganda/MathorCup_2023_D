...
class JobShopWithArgs:
    ...
    def solve(self):
        ...
        total_revenue, object = self.generate_qubo_model(excavator_numbers, fully_used_truck_numbers, half_used_excavator_bits, budget_constraint, truck_num_constraint)
        self.print_solution(object, total_revenue, excavator_numbers, fully_used_truck_numbers, half_used_excavator_bits, cost_con_s, budget_constraint, truck_num_constraint)
    ...
    def init_quantum_variables(self):
        ...
        cost_con_num = int2binary(self.data.total_budget - one_piece_cost)
        cost_con_s = self.qubo_util.convert_qubo_ndarray_from_bits(cost_con_num, 'cost_con_s')
    ...
    def generate_qubo_model(self, excavator_numbers, fully_used_truck_numbers, half_used_excavator_bits, budget_constraint, truck_num_constraint: dict):
        ...
        produce_expression_generator = (20 * (excavator_produce_efficiency[excavator_index] * qubo.make_qubo_ndarray_sum(excavator_numbers[f'excavator{excavator_index}']) - 0.5 * half_used_excavator_bits[f'excavator{excavator_index}_half_used']) 
                         for excavator_index in self.excavator_truck_dict.keys())
        ...
        total_revenue = 160 * 60 * qubo.kaiwu_sum_proxy(produce_expression_generator
                    ) - 160 * 7 * 60 * qubo.kaiwu_sum_proxy(oil_cost_expression_generator
                    ) - 60 * qubo.kaiwu_sum_proxy(labor_maintenance_cost_expression_generator
                    ) - 10000 * qubo.kaiwu_sum_proxy(excavator_precurement_cost_expression_generator)
                    
        object = -total_revenue + big_number * budget_constraint + qubo.kaiwu_sum_proxy(
            10000 * truck_num_constraint[f'excavator{excavator_index}_truck{truck_index}_constraint'] 
            for excavator_index, truck_index in self.excavator_truck_dict.items())
        return total_revenue, object
    
    def print_solution(self, obj, total_revenue, excavator_numbers: Dict[str, ndarray], fully_used_truck_numbers: Dict[str, ndarray], half_used_excavator_bits: dict, cost_con_s, budget_constraint, truck_num_constraint):
        qubo = self.qubo_util
